    ...
    return isUpdating ? <div className="flex justify-center flex-wrap items-center h-full w-full">
        <div role="status">
            ...
            <div className="w-full-text-center mt-8 font-bold">
                {downloadInfos.status === 'check' ? 'Vérification des mises à jour...' : downloadInfos.status === 'install' ? 'Installation de la dernière version...' : 'Téléchargement de la dernière version...'}
                {downloadInfos.status !== 'check' ? <div className="w-full bg-gray-200 rounded-full dark:bg-gray-700 mt-3">
                    <div className="bg-blue-600 text-xs font-medium text-blue-100 text-center p-0.5 leading-none rounded-full" style={{
                        width: `${Math.round(downloadInfos.percentage)}%`
                    }}> {Math.round(downloadInfos.percentage)}%</div>
                </div> : ''}
            </div>
        </div>
    </div> : <LoadingScreen />;
